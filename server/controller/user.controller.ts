    ...
    try {
      ...
      try {
        await sendMail({
          ...
          template: "reset-code.ejs",
          data,
        });
        res.status(200).json({
          success: true,
          message: `Reset code sent to ${user.email} successfully`,
        });
      } catch (error: any) {
...
export const verifyResetCode = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (
        ...
        (user.resetPasswordExpire && user.resetPasswordExpire.getTime() < Date.now())
      ) {
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (
        ...
        return next(new ErrorHandler("Reset session expired", 400));
      }

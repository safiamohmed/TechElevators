...
type Props = {
  ...
  setVideosToDelete?: (videosToDelete: string[]) => void;
  isEdit?: boolean;
};
...
const CourseContent: FC<Props> = ({
  ...
  const handleVideoChange = (index: number, file: File) => {
    ...
    video.onloadedmetadata = () => {
      const durationInSeconds = Math.floor(video.duration); // Store duration in seconds
      console.log("Duration in seconds:", durationInSeconds); // Debug log
      updatedData[index].videoLength = durationInSeconds; // Store as number
      setCourseContentData([...updatedData]);
  ...
  const newContentHandler = (item: any) => {
    if (
      ...
      const newContent = {
        ...
        videoLength: 0, // Initialize as 0 (number)
        links: [{ title: "", url: "" }],
        suggestion: "",
        _id: "",
      };
  ...
  const handleOptions = () => {
    if (
      ...
      setActive(active + 1);
      handleSubmit({ videosToDelete, courseContentData }); // Pass courseContentData to handleSubmit
    }
  };
                ...
                <>
                  ...
                  <div className="mb-3">
                    ...
                    <input
                      ...
                      className={`${styles.input}`}
                      value={formatDuration(item.videoLength)} // Display formatted duration
                      readOnly
                    />
                  </div>
                  <div className="mb-3">
